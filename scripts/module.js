...
Hooks.on("createActor", (actor, data, options, id) => {
  if (SETTINGS.autoAddActionGroup != "None") {
    ...
    if (!SETTINGS.disableLoot) addItemToActor(actor, [lootAction]);
  }
});

Hooks.on("dnd5e.preUseItem", function (item, config, options) {
  if (item.system.source != "Harvester") return;
  ...
  if (
    ...
    matchedItems.forEach((item) => {
      if (item.type == "loot") {
        ...
        else itemDC = item.system.source.match(/\d+/g)[0];
...
function addActionToActors() {
  ...
  game.actors.forEach((actor) => {
    ...
    if (!hasHarvest) addItemToActor(actor, [harvestAction]);
    if (!hasLoot && !SETTINGS.disableLoot) addItemToActor(actor, [lootAction]);
  });
  console.log(
